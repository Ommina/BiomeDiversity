...
public class NewFluidTest {
    ...
    //@ObjectHolder( "forge:test_fluid_source" )
    public static FlowingFluid test_fluid_source;
    ...
    public void registerBlocks( RegistryEvent.Register<Block> event ) {
        (test_fluid_source = new MyFlowingFluid.Source()).setRegistryName( "forge:test_fluid_source" );
        test_fluid_flowing = new MyFlowingFluid.Flowing().setRegistryName( "forge:test_fluid_flowing" );

        event.getRegistry().register(
             new FlowingFluidBlock( test_fluid_source, Block.Properties.create( Material.WATER ).doesNotBlockMovement().hardnessAndResistance( 100.0F ).noDrops() ) {
    ...
    public void registerItems( RegistryEvent.Register<Item> event ) {
        event.getRegistry().registerAll(
             new BucketItem( test_fluid_source, new Item.Properties().containerItem( Items.BUCKET ).maxStackSize( 1 ).group( ItemGroup.MISC ) ).setRegistryName( "forge:test_fluid_bucket" )
        );
    }

    public void registerFluids( RegistryEvent.Register<Fluid> event ) {
        event.getRegistry().registerAll( test_fluid_source, test_fluid_flowing );
    }
    ...
    private static abstract class MyFlowingFluid extends FlowingFluid {
        ...
        public boolean isEquivalentTo( Fluid fluidIn ) {
            return fluidIn == test_fluid_source || fluidIn == test_fluid_flowing;
        }
